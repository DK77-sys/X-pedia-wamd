...
const connectToWhatsApp = async (token, io = null ) => {
    ...
    if (typeof qrcode[token] !== 'undefined') {
        if (io !== null) {
            io.emit('qrcode', { token, data: qrcode[token], message: "Qrcode updated, please scann with your Whatsapp Device" })
        }
        return {
            status: false,
            sock: sock[token],
    ...
    sock[token] = makeWASocket({
        ...
        browser: ['X Pedia', 'Chrome', chrome?.data?.versions[0]?.version],
        logger,
    ...
    if(token) {
        ...
        if (fs.existsSync(path)) {
            if (typeof sock[token] === 'undefined') {
                const status = await connectWaBeforeSend(token)
                if (status) {
                  return  res.send({ status: true, message: `${token} connection was estabilished` })
                } else {
