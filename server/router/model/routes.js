...
const blast = async (req, res) => {
    ...
    asyncForEach(data, async (item) => {
        ...
        if (sender && receiver && message) {
            ...
            // if this first loop 
            if (sendingTextMessage) {
                const updateCampaign = await dbQuery(`UPDATE blasts SET status = 'success' WHERE receiver = '${receiver}' AND campaign_id = '${campaign_id}'`)
            } else {

                const updateCampaign = await dbQuery(`UPDATE blasts SET status = 'failed' WHERE receiver = '${receiver}' AND campaign_id = '${campaign_id}'`)

            }
        }
        await waitforme(req.body.delay * 1000)
    })

    return res.send({ status: true, message: 'On progress blasat,you can check the report at history blast page' })
